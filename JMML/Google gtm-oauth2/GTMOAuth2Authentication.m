...
// standard CTCT OAuth keys
static NSString *const kOAuth2AccessUsername       = @"username";
...
// If CJSONDeserializer is available, it is used for formatting JSON
@interface TouchJSONparser : NSObject
- (id)deserializeAsDictionary:(NSData *)data error:(NSError **)error;
@end
...
- (NSDictionary *)dictionaryWithJSONData:(NSData *)data {
  ...
  if (serializer) {
    ...
    return obj;
      ...
      // try TouchJSON
      Class jsonDeserializerClass = NSClassFromString(@"CJSONDeserializer");
      if (jsonDeserializerClass) {
          TouchJSONparser *Touchparser = [[[jsonDeserializerClass alloc] init] autorelease];
          obj = [Touchparser deserializeAsDictionary:data error:&error];
          return obj;
      }
...
- (NSString *)persistenceResponseString {
  ...
  [dict setValue:self.userName forKey:kOAuth2AccessUsername];
...
// CTCT custom key -- Username
- (NSString *)userName {
    return [self.parameters objectForKey:kOAuth2AccessUsername];
}

- (void)setUserName:(NSString *)str {
    [self.parameters setValue:str forKey:kOAuth2AccessUsername];
}
